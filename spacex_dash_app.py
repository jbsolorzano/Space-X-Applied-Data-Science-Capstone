...
def get_graph(entered_site):
    ...
    if entered_site=='ALL':
        ...
        fig = px.pie(pie_data, values=pie_data['class'], names=pie_data['Launch Site'], title='Successfull Launches Distributed by Launch Sites')
    else:
        ...
        fig = px.pie(pie_data, values=pie_data['Flight Number'], names=pie_data['class'], title='Success (1)/Failure (0) Launches for Site '+format(entered_site))
    return fig
